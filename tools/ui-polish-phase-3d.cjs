...
async function staticAudit() {
  ...
  for (const file of PAGES) {
    ...
    assert(`static-${file}-cache`, /product-ui\.css\?v=20260809u2/.test(html), 'front cache');
    assert(`static-${file}-shell-cache`, /front-shell\.js\?v=20260809u2/.test(html), 'shared shell retained');
  ...
  for (const viewport of VIEWPORTS) {
    for (const file of PAGES) {
      ...
      assert(`matrix-${file}-${viewport.name}-cache`,
        stylesheets.some((name) => name.includes('v=20260809u2')),
