...
async function pageGeometry(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      offenders,
      touch,
      cache: performance.getEntriesByType('resource').some((entry) => entry.name.includes('product-ui.css?v=20260809u2')),
...
async function staticAudit() {
  ...
  for (const file of PAGES) {
    ...
    assert(`${prefix}-cache`, /product-ui\.css\?v=20260809u2/.test(html), '20260809u2');
    assert(`${prefix}-namespace`, /ui-user-adoption-page/.test(html), 'body namespace');
    assert(`${prefix}-shell`, /front-shell\.js\?v=20260809u2/.test(html) && /user-workspace\.js\?v=20260809u2/.test(html), 'shared shell');
