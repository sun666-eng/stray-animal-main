...
foreach ($page in $shellPages) {
    ...
    Assert-True ($html -match 'href="\./index\.html"' -and $html -match '<footer\s+class="ui-site-footer"') "$($page.Path): public home link and verified footer shell"
}
...
Assert-True ($managerPayloadMatch.Success -and $managerPayloadMatch.Value -match 'status:\s*Number\(this\.manageForm\.status\)' -and $managerPayloadMatch.Value -match 'remark:\s*this\.manageForm\.remark' -and $managerPayloadMatch.Value -notmatch '\b(id|uid|uname|title|description|location|phone|pic|createTime|updateTime)\s*:' -and $adminHelpHtml -match "'/api/help/'\s*\+\s*encodeURIComponent\(view\.manageItem\.id\)\s*\+\s*'/manage'[\s\S]*?type:\s*'PUT'") "admin help: dedicated manager endpoint receives status and remark only"
...
Assert-True ($proofFrontHtml -match '/api/adopt/mine/' -and $proofFrontHtml -notmatch '/api/adopt/page2' -and $proofFrontHtml -match 'vstate\)===1|vstate\)\s*===\s*1' -and $proofFrontHtml -match '/api/files/staged/') "batch 7 owner proof: exact approved-relation preflight and staged upload cleanup"
Assert-True ($adminAdoptHtml -match 'canVisit\s*:\s*AdminWorkspace\.hasFlag\(user,\s*"visit"\)' -and $adminAdoptHtml -match 'vstate\)===1&&canVisit' -and ([regex]::Matches($adminAdoptHtml, "askAction\(item,'reject'\)").Count -ge 2)) "admin adopt: visit capability gate and mobile rejection parity"
