...
**状态**: 已封存并推送，提交 `09faaf7f40293ae3f2996fc7aa1faa833f9aef2f`
...
**封存**: 已提交并推送到 `origin/ui-polish/phase-3a-front-navigation-shell-20260731`，未合并 main。

### Phase 3B：公开发现与透明公示（2026-07-31）

**基线**: `09faaf7f40293ae3f2996fc7aa1faa833f9aef2f`（Phase 3A 封存）

**分支**: `ui-polish/phase-3b-public-discovery-20260731`

**测试端口**: `18121`（dev；收口后精确释放；9999 全程未触碰）

**状态**: 已封存并推送，提交 `4ad8ac7cbcb73db3273a380441b92a477856def8`

| 项 | 值 |
|----|-----|
| 页面范围 | 首页、动物浏览、动物详情、公告列表、公告详情、透明公示，共 6 个公开页面 |
| 视觉方向 | 暖白纸张、墨黑信息层级、朱红交互强调；保留 Phase 3A 全局导航壳 |
| 缓存 | 6 页 `product-ui.css?v=20260731b`；共享 shell/workspace 仍为 `v=20260731a` |
| Phase 3B | **507 / 0 / 0** strict；6×5 = **30/30** 页面矩阵；截图 **12** |
| 运行时探针 | **6**；best-effort=0 · fallback=0 · skipped=0 · 非 GET 写请求=0 |
| 错误门禁 | console error=0 · pageerror=0 · 未登记 HTTP=0 · requestfailed=0 |
| Phase 3A 回归 | **1227 / 0 / 0** strict；18×5 = 90/90 |
| 旧阶段回归 | 2H 131 · 2G 259 · 2F 139 · 2E 164 · 2D 224 · 2C 115 · 2B 234 · 2A 173 · 1C 134，全部 0 fail |
| 总门禁 | adversarial **853/0** · Maven **473/0/0** · git diff --check 0 |

**产品修改**:

- 首页改为可重试、可解释的真实聚合首页；状态徽标与 CTA 只依赖真实接口状态。
- 动物浏览统一搜索、类型筛选、结果摘要与分页语义；动物详情补齐参数校验、医疗信息失败态和不可领养说明。
- 公告列表与详情补齐无效 ID、未找到、长标题/长正文、失败重试和动态页面标题。
- 透明公示使用同源桌面表格与移动卡片，展示真实发生时间和业务分类；金额继续采用 `DecimalMoney` 精确字符串，不转浮点数。
- 公开列表/详情统一使用请求代次与 `AbortController`，慢请求晚到不能覆盖新结果；错误时清除旧数据，避免把旧内容伪装成当前结果。
- 本阶段没有修改后端、API、数据库、权限规则，也没有产生真实业务写请求。

**自检发现并修复**:

1. 移动筛选栏的搜索框继承 flex 尺寸后异常变高，已恢复为 48px 触控高度并验证 320/390px。
2. 极长动物名、公告标题与正文可能越界或产生孤字，已增加内容感知字号、断行和容器几何断言。
3. 超大精确金额虽然页面无横向滚动，但内容曾在局部容器中被裁切；已修复，并以精确文本与局部边界双重断言验证。
4. 动物、公告、资金请求均加入慢旧请求晚到对抗；无效详情 ID 在发请求前被拒绝。
5. 最终 P0/P1/P2/P3 均为 0。

权威报告: `output/playwright/ui-polish-phase-3b/PHASE-3B-REPORT.md` ·
`phase-3b-report.json` · `run-strict-final.log` · `screenshots-index.json`

**封存**: 已提交并推送到 `origin/ui-polish/phase-3b-public-discovery-20260731`，未合并 main。

### Phase 3C：用户领养、材料与回访闭环（2026-07-31）

**基线**: `4ad8ac7cbcb73db3273a380441b92a477856def8`（Phase 3B 封存）

**分支**: `ui-polish/phase-3c-user-adoption-workflow-20260731`

**测试端口**: `18122`（dev；收口后精确释放；9999 全程未触碰）

**状态**: 实现、自检、视觉复核与全量回归通过，**未提交、未推送、未进入 Phase 3D**

| 项 | 值 |
|----|-----|
| 页面范围 | 领养申请、我的领养、领养材料、回访记录，共 4 个用户闭环页面 |
| 视觉方向 | 沿用 Phase 3A/3B 暖白纸张、墨黑层级和朱红主操作；桌面与移动端信息层级一致 |
| 缓存 | 4 页 `product-ui.css?v=20260731c`；共享 shell/workspace 仍为 `v=20260731a` |
| Phase 3C | **333 / 0 / 0** strict；4×5 = **20/20** 页面矩阵；截图 **8** |
| 运行时探针 | **12**；best-effort=0 · fallback=0 · skipped=0 |
| 写入隔离 | 夹具受控写请求 15；真实业务写请求 **0** |
| 错误门禁 | console error=0 · pageerror=0 · 未登记 HTTP=0 · requestfailed=0 |
| 新阶段回归 | Phase 3B **507/0** · Phase 3A **1227/0** |
| 旧阶段回归 | 2H 131 · 2G 259 · 2F 139 · 2E 164 · 2D 224 · 2C 115 · 2B 234 · 2A 173 · 1C 134，全部 0 fail |
| 总门禁 | adversarial **853/0** · Maven **473/0/0** · git diff --check 0 |

**产品修改**:

- 领养申请加入 canonical ID、动物与既有申请前置校验、字段级错误、写锁和 409 输入保留；材料待补状态不再被误判为可重复申请。
- “我的领养”加入 latest-wins 列表与时间线、失败清空旧数据、可重试错误态，以及带 Esc、焦点恢复和双击锁的撤回确认。
- 领养材料实现“上传成功不等于业务保存成功”的暂存状态机；失败可原 flag 重试，替换、取消和离页只清理一次，业务成功不误删已绑定文件。
- 回访计划与回访记录使用独立 loading/error/seq/abort，互不覆盖；页面只展示真实记录，未记录的回访方式不作推断。
- 本阶段没有修改后端、API、数据库或权限规则；所有真实服务验证均只读。

**自检发现并修复**:

1. 补齐暂存材料被替换与重复 `pagehide` 的运行时探针；严格套件曾因错误等待离页 DOM 更新而失败，改为直接验证 DELETE 恰好一次。
2. 全局对抗契约曾把旧代码空格、局部变量名和旧文案误当安全语义；保留原门禁目标并升级正则后由 850/3 恢复为 853/0。
3. 逐张检查 1440/390 共 8 张截图，长标题、长地址、材料与回访内容均无局部或页面横向溢出。
4. 最终 P0/P1/P2/P3 均为 0。

权威报告: `output/playwright/ui-polish-phase-3c/PHASE-3C-REPORT.md` ·
`phase-3c-report.json` · `run-strict-final.log` · `screenshots-index.json`

**停止**: 不提交、不推送、不合并 main、不进入 Phase 3D。等待用户确认。
